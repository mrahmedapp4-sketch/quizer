    ...
    <div className="min-h-screen p-4 md:p-8 relative">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <section className="lg:col-span-12 bg-zinc-950/80 backdrop-blur-xl rounded-3xl p-6 border border-amber-400/20 shadow-xl">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-400">لوحة المراقب: بحث، نقاط، وإدارة الحسابات المحفوظة</p>
                </div>
              </div>
              <Button
                type="button"
                ...
                className="bg-red-500/15 hover:bg-red-500/25 text-red-300 border border-red-400/30 rounded-xl font-bold"
              >
                <Trash2 className="w-4 h-4 ml-2" />
                {isResettingDatabase ? "جاري التنظيف..." : "تنظيف قاعدة البيانات"}
              </Button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
              <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
                <div className="text-xs text-zinc-400">كل الحسابات</div>
                <div className="text-2xl font-black text-white">{databaseStudents.length}</div>
              </div>
              <div className="rounded-2xl bg-green-500/10 border border-green-400/20 p-4">
                <div className="text-xs text-green-300">الحسابات النشطة</div>
                <div className="text-2xl font-black text-green-200">{databaseStudents.filter((student) => !student.archivedAt).length}</div>
              </div>
              <div className="rounded-2xl bg-zinc-500/10 border border-zinc-400/20 p-4">
                <div className="text-xs text-zinc-400">الحسابات المؤرشفة</div>
                <div className="text-2xl font-black text-zinc-200">{databaseStudents.filter((student) => student.archivedAt).length}</div>
              </div>
            </div>
            ...
            <div className="relative mb-5">
              ...
              <Input
                ...
                placeholder="ابحث باسم الطالب أو اسم المستخدم أو البريد..."
                className="h-12 pr-12 bg-white/5 border-white/10 text-white placeholder:text-zinc-500 rounded-xl"
              />
            </div>
            ...
            <div className="overflow-x-auto rounded-2xl border border-white/10">
              <table className="w-full text-right min-w-[760px]">
                ...
                <tbody>
                  {filteredDatabaseStudents.map((student) => (
                    <tr key={student.id} className="border-b border-white/5 last:border-0 hover:bg-white/5">
                      ...
                      <td className="p-4">
                        <span className={cn("rounded-full px-3 py-1 text-xs font-bold", student.archivedAt ? "bg-zinc-700 text-zinc-300" : "bg-green-500/15 text-green-300")}>
                          {student.archivedAt ? "مؤرشف" : "نشط"}
                        </span>
                      </td>
                      <td className="p-4">
                        {!student.archivedAt ? (
                          <div className="flex items-center justify-center gap-2">
                            <Button type="button" onClick={() => handleAdminPoint(student.id)} className="h-9 rounded-lg bg-primary/15 hover:bg-primary/25 text-primary border border-primary/30">
                              <Plus className="w-4 h-4 ml-1" /> نقطة
                            </Button>
                            <Button type="button" onClick={() => { setPasswordStudent(student); setNewPassword(""); }} className="h-9 rounded-lg bg-white/10 hover:bg-white/20 text-white">
                              <KeyRound className="w-4 h-4 ml-1" /> كلمة مرور
                            </Button>
                            <Button
                              type="button"
                              onClick={async () => {
                              ...
                              className="h-9 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-400/20"
                            >
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </div>
                        ) : (
                          <span className="text-xs text-zinc-500">لا توجد إجراءات</span>
                        )}
                      </td>
                    </tr>
                  ))}
                  {filteredDatabaseStudents.length === 0 && (
                    <tr>
                      <td colSpan={6} className="p-10 text-center text-zinc-500">لا يوجد طلاب مطابقون للبحث</td>
                    </tr>
                  )}
                </tbody>
              </table>
            </div>
            <p className="mt-4 text-xs text-zinc-500">لأمان الحسابات، كلمات المرور لا تظهر حتى للمراقب. استخدم زر «كلمة مرور» لتعيين كلمة جديدة.</p>
          </section>
